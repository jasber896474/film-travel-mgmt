...
const T = {
  "zh-TW": {
    ...
    hotelStatsByDateHint: "請先選擇飯店與日期，分開計算各飯店當晚人數、房數與費用（費用依主檔「日期別房價」，同房號或同室者只計一筆）。",
...
function computeHotelStatsForDate(stays, roommates, dateStr, pricingRules) {
  ...
  stays.forEach((s) => {
    ...
    byHotel[hid].guestIds.add(s.person_id);
...
function PersonForm({ init, onSave, onClose, t, lang }) {
  const blank = { dept: "", title_kanji: "", title_other: "", name_kanji: "", last_roman: "", first_roman: "", importance: 0, passport: "", dob: "", passport_exp: "", diet: "" };
  const [f, setF] = useState(init ? { ...blank, ...init, title_kanji: init.title_kanji || "", title_other: init.title_other || "" } : blank);
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <Field label={t.hotel}><select style={inpStyle} value={f.hotel_id} onChange={set("hotel_id")}><option value="">--</option>{hotels.map((h) => <option key={h.id} value={String(h.id)}>{h.name}</option>)}</select></Field>
        ...
        <Field label={t.basePrice}><input type="number" min="0" style={inpStyle} value={f.base_price} onChange={set("base_price")} /></Field>
      ...
      {breakdown.length > 0 && (
        <div style={{ marginTop: 10, background: "var(--washi)", padding: 10 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {breakdown.map((b) => <span key={b.date} style={{ fontSize: 10, padding: "3px 8px", border: "1px solid var(--keisenL)" }}>{b.date.slice(5)} {formatMoney(convertMoney(b.price, project), project, lang)}</span>)}
          </div>
        </div>
      )}
...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const personsWithNoTrips = useMemo(() => {
    ...
    return persons.filter((p) => !has.has(p.id));
  ...
  const flatStays = useMemo(() => {
    ...
    stays.forEach((s) => {
      ...
      const nameOk = !kw || [p.name_kanji, p.last_roman, p.dept].some((v) => (v || "").toLowerCase().includes(kw));
                    ...
                    {rowsA.map((p, i) => {
                      ...
                      return (
                        <tr key={p.id} draggable={canEdit} onDragStart={() => { dragItem.current = persons.indexOf(p); }} onDragEnter={() => { dragOver.current = persons.indexOf(p); }} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()}>
                          ...
                          <td className="col-dept" style={tdEllipsis()} title={p.dept}><Highlight text={p.dept} query={debSearchA} /></td>
                ...
                <>
                  ...
                  {(() => {
                    ...
                    const stayHeaderDefs = [
                      ...
                      { key: "kanji", label: t.nameKanji },
                      { key: "roman", label: t.nameEnglish },
                      { key: "seg", label: t.staySegment },
                      { key: "room", label: t.roomNo },
                      { key: "type", label: t.roomType },
                      { key: "in", label: t.checkIn },
                      { key: "out", label: t.checkOut },
                      { key: "nights", label: t.nights },
                      { key: "amt", label: t.totalAmt },
                      { key: "act", label: t.action },
                    ];
                    ...
                    const stayRow = (row) => {
                      ...
                      return (
                        <tr key={row.id} style={{ background: groupColor ? `${groupColor}0d` : "transparent" }}>
                          ...
                          <td style={tdS({ textAlign: "center", maxWidth: "none" })}><RomanNameCell last={row.person.last_roman} first={row.person.first_roman} /></td>
                          ...
                          <td className="td-amt col-center" style={tdS({ maxWidth: "none", fontWeight: 600, color: isZeroPrice ? "var(--usunezumi)" : "var(--asagi)" })}>
                            {isZeroPrice ? "—" : formatMoney(convertMoney(row.total_amount, project), project, lang)}
                          </td>
                          ...
                          return (
                            <section key={String(group.hotelId)} style={{ marginBottom: 32 }}>
                              <div style={{
                                ...
                                <span style={{ fontSize: 11, color: "var(--nezumi)" }}>
                              ...
                              <div style={{ overflowX: "auto" }}>
                                <table style={tblW} className="data-table stay-table">
                                  ...
                                  <thead><tr style={thead}>
                                    {stayHeaderDefs.map(({ key, label }) => (
                                      <th key={key} style={thS}>{label}</th>
                                    ))}
                                  </tr></thead>
                                  <tbody>
