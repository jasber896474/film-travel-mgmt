...
const T = {
  ...
  en: {
    ...
    subStay: "Stay List", subHotels: "Hotel Master",
  ...
  ko: {
    ...
    subStay: "숙박 목록", subHotels: "호텔 마스터",
...
function PersonForm({ init, onSave, onClose, t, lang }) {
  const blank = { dept: "", title_kanji: "", title_other: "", name_kanji: "", last_roman: "", first_roman: "", importance: 0, passport: "", dob: "", passport_exp: "", diet: "", on_site: true };
  const [f, setF] = useState(init ? { ...blank, ...init, title_kanji: init.title_kanji || "", title_other: init.title_other || "", on_site: init.on_site !== false } : blank);
  ...
  return (
    <div>
      ...
      {/* on_site toggle */}
      <label style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 16, padding: "10px 14px", background: "var(--washi)", borderRadius: 6, border: "1px solid var(--keisenL)", cursor: "pointer" }}>
        <input type="checkbox" checked={f.on_site !== false} onChange={(e) => setF((p) => ({ ...p, on_site: e.target.checked }))} style={{ width: 16, height: 16, accentColor: "var(--moegi)", cursor: "pointer" }} />
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: f.on_site !== false ? "var(--moegi)" : "var(--nezumi)" }}>{t.onSite}</div>
          <div style={{ fontSize: 10, color: "var(--usunezumi)", marginTop: 2 }}>{t.offSiteHint}</div>
        </div>
      </label>
...
function generateReportHTML({ type, cols, data, project, lang, t }) {
  ...
  const accentColor = "#4a8f6a";

  const thStyle = `padding:7px 10px;font-size:8.5pt;font-weight:700;letter-spacing:.08em;text-align:left;border-right:1px solid rgba(255,255,255,.15);white-space:nowrap;`;
  const tdStyle = `padding:6px 10px;font-size:9.5pt;border-bottom:1px solid #e0ebe4;border-right:1px solid #e0ebe4;vertical-align:top;`;
  ...
  const rows = data.map((row, i) => {
    ...
    const cells = cols.map((c) => `<td style="${tdStyle}background:${bg};">${row[c.key] ?? "—"}</td>`).join("");
    return `<tr>${cells}</tr>`;
  });
...
body{font-family:'Noto Sans JP',sans-serif;font-size:10pt;color:#1a2820;background:#fff;}
@page{size:A4 ${pageDir};margin:8mm 10mm;}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}
.wrap{padding:2mm;}
.doc-header{background:${headerGrad};color:#fff;padding:14px 18px;border-radius:6px;display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;}
.doc-title{font-family:'Noto Serif JP',serif;font-size:14pt;font-weight:600;letter-spacing:.04em;}
.doc-meta{font-size:8pt;opacity:.75;text-align:right;line-height:1.7;}
.doc-type{display:inline-block;background:rgba(255,255,255,.18);padding:3px 10px;border-radius:12px;font-size:8.5pt;font-weight:700;letter-spacing:.08em;margin-bottom:6px;}
table{width:100%;border-collapse:collapse;border-radius:4px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.08);}
thead{background:${headerGrad};color:#fff;}
.footer{margin-top:12px;font-size:7.5pt;color:#888;text-align:right;border-top:1px solid #d8e8dc;padding-top:6px;}
</style></head><body><div class="wrap">
<div class="doc-header"><div><div class="doc-type">${typeLabel}</div><div class="doc-title">${projectName}</div></div><div class="doc-meta"><div>${now}</div><div>${data.length} ${t.resultsCount ? "" : "rows"}</div><div style="font-size:7pt;margin-top:4px;opacity:.6;">${isLandscape ? "A4 Landscape" : "A4 Portrait"}</div></div></div>
...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [p, f, s, h, pr, rm, vh, va] = await Promise.all([
  ...
  const personsWithNoTrips = useMemo(() => {
    ...
    return persons.filter((p) => p.on_site !== false && !has.has(p.id));
  ...
  const flatStays = useMemo(() => {
    ...
    stays.forEach((s) => {
      ...
      if (nameOk && hotelOk && roomOk && deptOk) {
                      ...
                      return (
                        <tr key={p.id} draggable={canEdit} onDragStart={() => { dragItem.current = persons.indexOf(p); }} onDragEnter={() => { dragOver.current = persons.indexOf(p); }} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()}>
                          ...
                          <td className="col-dept" style={tdEllipsis()} title={p.dept}>
                            ...
                            {p.on_site === false && <span style={{ marginLeft: 4, fontSize: 8, padding: "1px 5px", borderRadius: 3, background: "rgba(173,48,48,.1)", color: "var(--shu)", fontWeight: 700, whiteSpace: "nowrap", verticalAlign: "middle" }}>off</span>}
                          </td>
                  ...
                  <div style={{
                    ...
                    {canEdit && (
                      <select style={{ ...inpStyle, width: "100%" }} className="wa-input" value={addStayPerson} onChange={(e) => setAddStayPerson(e.target.value)}>
                        ...
                        {persons.filter((p) => p.on_site !== false).map((p) => <option key={p.id} value={p.id}>{p.name_kanji || p.last_roman}</option>)}
                      </select>
                    )}
                    {canEdit && (
                        ...
                        return (
                          <div key={v.id} style={{ background: "var(--shiro)", borderRadius: 8, border: "1px solid var(--keisenL)", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.05)" }}>
                            ...
                            <div style={{ padding: "12px 14px" }}>
                              {assigns.length > 0 && (
                                <div style={{ fontSize: 11, color: "var(--nezumi)", marginBottom: 8, paddingBottom: 8, borderBottom: "1px solid var(--keisenL)" }}>
                                  {t.onboardDepts}: <span style={{ fontWeight: 600, color: "var(--sumi)" }}>{vehicleDeptSummary(assigns, persons) || "—"}</span>
                                </div>
                              )}
                              {assigns.map((a) => {
                                const p = persons.find((x) => x.id === a.person_id);
                                if (!p) return null;
                                return (
                                  <div key={a.id || `${a.person_id}-${a.role}-${a.segment_label}`} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid var(--keisenL)", fontSize: 12 }}>
                                    <div>
                                      <span style={{ fontWeight: 600, color: "var(--sumi)" }}>{p.dept || "—"}</span>
                                      <span style={{ fontSize: 11, color: "var(--nezumi)", marginLeft: 8 }}>{p.name_kanji}</span>
                                      {a.role === "driver" && <span style={{ fontSize: 9.5, color: "var(--moegi)", fontWeight: 600, marginLeft: 6, padding: "1px 5px", background: "var(--moegi3)", borderRadius: 3 }}>{t.driver}</span>}
                                    </div>
                              ...
                              {canEdit && (
                                <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                                  <select style={{ ...inpStyle, flex: "2 1 120px", minWidth: 0 }} value={addTripVehicle === v.id ? tripPerson : ""} onChange={(e) => { setAddTripVehicle(v.id); setTripPerson(e.target.value); }}>
                                    ...
                                    {persons.filter((p) => p.on_site !== false).map((p) => <option key={p.id} value={p.id}>{p.dept ? `[${p.dept}] ` : ""}{p.name_kanji || `${p.last_roman} ${p.first_roman}`.trim()}</option>)}
                                  </select>
                                  ...
                                  {addTripVehicle === v.id && tripPerson && (() => {
                                    ...
                                    return sel?.dept ? <span style={{ fontSize: 11, fontWeight: 600, color: "var(--moegi)", width: "100%" }}>{t.dept}: {sel.dept}</span> : null;
                                  })()}
                                </div>
                              )}
                            </div>
