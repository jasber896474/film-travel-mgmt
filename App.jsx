...
const T = {
  "zh-TW": {
    ...
    no: "編號", dept: "部門", nameKanji: "姓名（漢字）", nameRoman: "羅馬拼音", nameSurname: "姓", nameGiven: "名", importance: "重要度",
    status: "安排狀態", passport: "護照 / ID Card", dob: "出生年月日", passportExp: "護照效期", diet: "飲食限制", action: "操作", select: "選取",
    ...
    hotelStatsByDateHint: "人數與房數依住宿一覽；當日費用依飯店主檔「日期別房價」，同房號或同室者只計一筆（以該房第一位在住者的房型對應當日房費）。",
    statsPickDate: "選擇日期", statsDaySummary: "{date}：{guests} 人 · {rooms} 間房 · {cost}（{hotels} 間飯店）", noStayDates: "尚無住宿日期可統計",
  ...
  "zh-CN": {
    ...
    no: "编号", dept: "部门", nameKanji: "姓名（汉字）", nameRoman: "罗马拼音", nameSurname: "姓", nameGiven: "名", importance: "重要度",
    status: "安排状态", passport: "护照 / ID Card", dob: "出生年月日", passportExp: "护照效期", diet: "饮食限制", action: "操作", select: "选取",
    ...
    hotelStatsByDateHint: "人数与房数依住宿一览；当日费用依饭店主档「日期别房价」，同房号或同室者只计一笔。",
    statsPickDate: "选择日期", statsDaySummary: "{date}：{guests} 人 · {rooms} 间房 · {cost}（{hotels} 间饭店）", noStayDates: "尚无住宿日期可统计",
  ...
  en: {
    ...
    no: "No.", dept: "Dept", nameKanji: "Name", nameRoman: "Romanized", nameSurname: "Last", nameGiven: "First", importance: "Priority",
    status: "Status", passport: "Passport / ID Card", dob: "DOB", passportExp: "Expiry", diet: "Dietary", action: "Actions", select: "Select",
    ...
    hotelStatsByDateHint: "Auto-calculated from the stay list. Pick a date to see crew headcount and room count per hotel that night.",
    statsPickDate: "Date", statsDaySummary: "{date}: {guests} guests · {rooms} rooms · {cost} ({hotels} hotel(s))", noStayDates: "No stay dates to summarize",
  ...
  ko: {
    ...
    no: "번호", dept: "부서", nameKanji: "이름", nameRoman: "로마자", nameSurname: "성", nameGiven: "이름", importance: "중요도",
    status: "상태", passport: "여권 / ID Card", dob: "생년월일", passportExp: "만료", diet: "식단", action: "작업", select: "선택",
    ...
    hotelStatsByDateHint: "숙박 목록에서 자동 계산합니다. 날짜를 선택하면 해당 밤 호텔별 인원·객실 수가 표시됩니다.",
    statsPickDate: "날짜 선택", statsDaySummary: "{date}: {guests}명 · {rooms}실 · {cost} ({hotels}개 호텔)", noStayDates: "통계할 숙박 일자가 없습니다",
  ...
  ja: {
    ...
    no: "No.", dept: "部署", nameKanji: "氏名", nameRoman: "ローマ字", nameSurname: "姓", nameGiven: "名", importance: "重要度",
    status: "状況", passport: "パスポート / ID", dob: "生年月日", passportExp: "有効期限", diet: "食事制限", action: "操作", select: "選択",
    ...
    hotelStatsByDateHint: "宿泊一覧から自動集計。日付を選ぶと、その夜のホテル別人数・部屋数を表示します。",
    statsPickDate: "日付", statsDaySummary: "{date}：{guests}名・{rooms}室・{cost}（{hotels}ホテル）", noStayDates: "集計できる宿泊日がありません",
...
function NameColumnHeader({ t }) {
  return (
    <div className="roman-name-cell" style={{ lineHeight: 1.3, textAlign: "center", fontSize: 8 }}>
      <div>{t.nameSurname}</div>
      <div>{t.nameGiven}</div>
    </div>
  );
}
...
const STAY_COL_WIDTHS = [32, 80, 96, 58, 64, 60, 50, 68, 68, 34, 70, 88];
  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <Field label={`${t.diet} (${t.optional})`}><input style={inpStyle} value={f.diet || ""} onChange={set("diet")} /></Field>
      </div>
...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const hotelStatsDayRows = useMemo(() => hotels
    .map((h) => ({ hotel: h, st: hotelStatsByDateMap[h.id] || { guests: 0, rooms: 0, cost: 0 } }))
    .filter(({ st }) => st.guests > 0)
    .sort((a, b) => b.st.guests - a.st.guests || (a.hotel.name || "").localeCompare(b.hotel.name || "")),
  [hotels, hotelStatsByDateMap]);
  const hotelStatsDayTotals = useMemo(() => hotelStatsDayRows.reduce(
    (acc, { st }) => ({
      guests: acc.guests + st.guests,
      rooms: acc.rooms + st.rooms,
      cost: acc.cost + st.cost,
      hotels: acc.hotels + 1,
    }),
    { guests: 0, rooms: 0, cost: 0, hotels: 0 },
  ), [hotelStatsDayRows]);
  const hotelStatsDayCostFmt = useMemo(
    () => formatMoney(convertMoney(hotelStatsDayTotals.cost, project), project, lang),
    [hotelStatsDayTotals.cost, project, lang],
  );
  useEffect(() => {
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={tblW} className="data-table staff-table">
                  ...
                  <thead><tr style={thead}>
                    ...
                    <th style={{ ...thS, textAlign: "center" }}><NameColumnHeader t={t} /></th>
            ...
            <div className="tab-content">
              ...
              {accSub === "stays" && (
                <>
                            ...
                            <section key={String(group.hotelId)} style={{ marginBottom: 32 }}>
                              ...
                              <div style={{ overflowX: "auto" }}>
                                <table style={tblW} className="data-table stay-table">
                                  ...
                                  <thead><tr style={thead}>
                                    {stayHeaderDefs.map(({ key, label, roman }) => (
                                      <th key={key} style={thS}>{roman ? <NameColumnHeader t={t} /> : label}</th>
                                    ))}
                                  </tr></thead>
                                  <tbody>
                  ...
                  <div style={{ marginTop: 28, padding: "16px 18px", background: "var(--washi)", borderRadius: 8, border: "1px solid var(--keisenL)" }}>
                    ...
                    {statsDate && hotelStatsDayRows.length > 0 && (
                      <>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10, marginBottom: 12 }}>
                          {[
                            { label: t.guestCountDay, value: hotelStatsDayTotals.guests },
                            { label: t.roomCountDay, value: hotelStatsDayTotals.rooms },
                        ...
                        <p style={{ fontSize: 12, fontWeight: 600, color: "var(--moegi)", margin: "0 0 12px" }}>
                          {t.statsDaySummary
                            .replace("{date}", fmtDate(statsDate, lang))
                            .replace("{guests}", String(hotelStatsDayTotals.guests))
                            .replace("{rooms}", String(hotelStatsDayTotals.rooms))
                            .replace("{cost}", hotelStatsDayCostFmt)
                            .replace("{hotels}", String(hotelStatsDayTotals.hotels))}
                        </p>
                      </>
                    )}
                    {statsDate && hotelStatsDayRows.length === 0 && (
                    ...
                    {statsDate && hotelStatsDayRows.length > 0 && (
                      <div style={{ overflowX: "auto", background: "var(--shiro)", borderRadius: 6, border: "1px solid var(--keisenL)" }}>
                        <table style={tblW} className="data-table hotel-stats-table">
                          ...
                          <thead><tr style={thead}>
                            {[
                              ...
                              { h: t.guestCountDay, align: "right" },
                              { h: t.roomCountDay, align: "right" },
                              { h: t.totalSpendDay, align: "right" },
                            ].map(({ h, align }) => <th key={h} style={{ ...thS, textAlign: align }}>{h}</th>)}
                          </tr></thead>
                          <tbody>
                            {hotelStatsDayRows.map(({ hotel, st }) => (
                              <tr key={hotel.id}>
                                <td style={tdEllipsis({ fontWeight: 600 })} title={hotel.name}>{hotel.name}</td>
                                ...
                                <td className="td-amt" style={tdS({ textAlign: "right", fontVariantNumeric: "tabular-nums", fontWeight: 600 })}>
                                  {formatMoney(convertMoney(st.cost, project), project, lang)}
                                </td>
                              </tr>
                            ))}
                          </tbody>
