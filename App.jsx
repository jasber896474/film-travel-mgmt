...
function VehicleForm({ init, onSave, onClose, t }) {
  ...
  const [f, setF] = useState(() => {
    ...
    return { ...blank, ...init, regionKey: init.region_key || init.regionKey || "TW", typeKey: tk, capacity: init.capacity || VEHICLE_DEFAULT_CAPACITY[tk] || 4 };
  });
      ...
      <div style={{ marginTop: 16, padding: "12px 14px", background: "var(--washi)", borderRadius: 6, border: "1px solid var(--keisenL)" }}>
        ...
        <div style={inp2}>
          <Field label={t.pickupTime}>
            <input style={inpStyle} type="time" value={f.pickup_time} onChange={(e) => setF((p) => ({ ...p, pickup_time: e.target.value }))} />
          </Field>
          <Field label={t.vehicleArrivalTime}>
            <input style={inpStyle} type="time" value={f.arrival_time} onChange={(e) => setF((p) => ({ ...p, arrival_time: e.target.value }))} />
          </Field>
        </div>
        <div style={inp2}>
          <Field label={t.pickupLocation}>
            <input style={inpStyle} value={f.pickup_location} onChange={(e) => setF((p) => ({ ...p, pickup_location: e.target.value }))} maxLength={60} />
          </Field>
...
function generateReportHTML({ type, cols, data, project, lang, t }) {
  ...
  // Vehicle report: group by vehicle, inline numbered passengers
  if (type === "vehicle") {
...
function ReportExportModal({ onClose, t, lang, project, persons, flights, stays, hotels, pricingRules, vehicles, vehicleAssignments, personIndex, stayDisplayTotals }) {
  ...
  return (
    <Modal title={t.reportExport} onClose={onClose}>
      ...
      <div style={{ marginBottom: 10, fontSize: 11, fontWeight: 700, color: "var(--nezumi)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{t.selectCols}</div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
        <button type="button" onClick={onClose} style={{ padding: "8px 16px", border: "1px solid var(--keisenM)", background: "var(--shiro)", cursor: "pointer" }}>{t.cancel}</button>
        <button type="button" onClick={handlePreview} disabled={selCount === 0} style={pBtn(selCount === 0)}>{t.previewReport}</button>
      </div>
    </Modal>
  );
}
                        ...
                        return (
                          <div key={v.id} style={{ background: "var(--shiro)", borderRadius: 8, border: "1px solid var(--keisenL)", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.05)" }}>
                            ...
                            <div style={{ background: "var(--sumiMid)", color: "var(--washi)", padding: "14px 18px" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                <div>
                                  <strong style={{ fontSize: 14, fontFamily: "'Noto Serif JP',serif" }}>{v.name}</strong>
                            ...
                            {(v.vehicle_no || v.driver_phone || v.license_plate || v.pickup_time || v.pickup_location || v.arrival_time || v.arrival_location || v.note) && (
                              <div style={{ padding: "8px 14px", background: "var(--washi)", borderBottom: "1px solid var(--keisenL)", fontSize: 11.5, color: "var(--sumi)", lineHeight: 1.7 }}>
                                ...
                                {formatVehicleTravelDuration(v.pickup_time, v.arrival_time, lang) && (
                                  <span style={{ marginRight: 12, fontWeight: 600 }}>{t.travelDuration}: {formatVehicleTravelDuration(v.pickup_time, v.arrival_time, lang)}</span>
                                )}
                                ...
                                {v.note && <span style={{ color: "var(--nezumi)" }}>{v.note}</span>}
                              </div>
                            )}
                              ...
                              return (
                                <div key={v.id} style={{ background: "var(--shiro)", border: "1px solid var(--keisenL)", borderRadius: 8, overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,.05)" }}>
                                  <div style={{ background: "var(--sumiMid)", backgroundImage: "var(--theadGrad)", padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <div>
                                      <strong style={{ fontSize: 14, fontFamily: "'Noto Serif JP',serif", color: "var(--washi)" }}>{v.name}</strong>
