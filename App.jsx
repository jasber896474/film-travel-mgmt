...
function HotelStayForm({ init, hotels, pricingRules, onSave, onClose, t, project, lang, roommateNames, isSpecialRoom }) {
  const blank = { hotel_id: "", room_type: "Single", room_custom: "", check_in: todayStr(), check_out: (() => { const d = new Date(); d.setDate(d.getDate() + 1); return localDateStr(d); })(), base_price: "", stay_label: "", room_number: "", special_room_name: "", location_tag: "" };
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          <Field label={t.locationTag}>
            <input style={inpStyle} value={f.location_tag || ""} onChange={set("location_tag")} placeholder={t.locationTagPh} />
          </Field>
        </div>
      </div>
      {breakdown.length > 0 && (
...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const [filterLocationTag, setFilterLocationTag] = useState("");
  ...
  const flatStays = useMemo(() => {
    ...
    stays.forEach((s) => {
      ...
      const locOk = !filterLocationTag || (s.location_tag || "") === filterLocationTag;
      if (nameOk && hotelOk && roomOk && deptOk && locOk) {
    ...
    return rows;
  }, [stays, persons, debSearchC, searchCHotel, searchCRoom, deptC, filterLocationTag, personIndex, hotels]);
              ...
              {accSub === "stays" && (
                <>
                  <div style={{
                    ...
                    {/* location_tag filter */}
                    {(() => {
                      const tags = [...new Set(stays.map((s) => s.location_tag).filter(Boolean))].sort();
                      if (!tags.length) return null;
                      return (
                        <select style={{ ...inpStyle, width: "100%" }} value={filterLocationTag} onChange={(e) => setFilterLocationTag(e.target.value)}>
                            ...
                            <section key={String(group.hotelId)} style={{ marginBottom: 32 }}>
                              <div style={{
                                ...
                                <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
                                  ...
                                  {cs.guestNights > 0 && <span style={{ fontSize: 11, color: "var(--nezumi)" }}>{t.guestNights}: <strong style={{ color: "var(--sumi)" }}>{cs.guestNights}</strong></span>}
