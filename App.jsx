...
const T = {
  "zh-TW": {
    ...
    hideFromStays: "隱藏於住宿名單", hideFromVehicles: "隱藏於派車名單",
  ...
  "zh-CN": {
    ...
    hideFromStays: "隐藏于住宿名单", hideFromVehicles: "隐藏于派车名单",
  ...
  en: {
    ...
    hideFromStays: "Hide from stays", hideFromVehicles: "Hide from vehicles",
  ...
  ko: {
    ...
    hideFromStays: "숙박 목록에서 숨김", hideFromVehicles: "차량 목록에서 숨김",
  ...
  ja: {
    ...
    hideFromStays: "宿泊リストから非表示", hideFromVehicles: "配車リストから非表示",
...
function ProgressBar({ t, stats }) {
  const items = [
    { label: t.progressFlight, done: stats.flightDone, total: stats.total, color: "var(--asagi)" },
    { label: t.progressHotel, done: stats.hotelDone, total: stats.total, color: "var(--kincha)" },
    { label: t.progressVehicle, done: stats.vehicleAssigned, total: stats.total, color: "var(--moegi)", inverse: true },
  ];
  return (
...
function PersonForm({ init, onSave, onClose, t, lang }) {
  const blank = { dept: "", title_kanji: "", title_other: "", name_kanji: "", last_roman: "", first_roman: "", importance: 0, passport: "", dob: "", passport_exp: "", diet: "", hide_from_stays: false, hide_from_vehicles: false };
  const [f, setF] = useState(init ? { ...blank, ...init, title_kanji: init.title_kanji || "", title_other: init.title_other || "", hide_from_stays: !!init.hide_from_stays, hide_from_vehicles: !!init.hide_from_vehicles } : blank);
    ...
    if (reportType === "flight") {
      return persons.map((p) => {
        ...
        return {
          ...
          dep_time: fl?.dep_time ? fl.dep_time.slice(0, 16).replace("T", " ") : "—",
          arr_airport: fl?.arr_airport || "—",
          arr_time: fl?.arr_time ? fl.arr_time.slice(0, 16).replace("T", " ") : "—",
        };
      });
    }
    if (reportType === "staff") {
...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const progressStats = useMemo(() => {
    ...
    const flightDone = persons.filter((p) => { const fl = flights.find((x) => x.person_id === p.id); return fl && fl.airline; }).length;
    const hotelDone = persons.filter((p) => stays.some((s) => s.person_id === p.id && s.hotel_id)).length;
    const tripPersons = new Set(allAssignments.map((a) => a.person_id));
    return { total, flightDone, hotelDone, vehicleAssigned: tripPersons.size, unassignedVehicle: persons.filter((p) => !tripPersons.has(p.id)).length };
  ...
  const exportFlightCSV = () => {
    const headers = [t.no, t.dept, t.nameKanji, t.nameRoman, t.airline, t.flightNo, t.cabin, t.pnr, t.depAirport, t.depTime, t.arrAirport, t.arrTime, t.status];
    const rows = persons.map((p, i) => {
      const fl = flights.find((x) => x.person_id === p.id);
      ...
      return [personIndex[p.id], p.dept, p.name_kanji, `${p.last_roman} ${p.first_roman}`.trim(),
        ...
        fl?.dep_time ? new Date(fl.dep_time).toLocaleString() : "",
        fl?.arr_airport ? `${fl.arr_airport}${arrT ? " "+arrT : ""}` : "",
        fl?.arr_time ? new Date(fl.arr_time).toLocaleString() : "",
        fl?.airline ? t.arranged_short : t.unArranged_short];
    });
        ...
        <div style={{ padding: "28px 20px", maxWidth: 1320, margin: "0 auto" }}>
          ...
          {tab === "B" && (
            <div className="tab-content">
              ...
              <p style={{ fontSize: 11, color: "var(--nezumi)", marginBottom: 10, letterSpacing: "0.03em" }}>{t.resultsCount.replace("{n}", filteredFlightPersons.length).replace("{total}", persons.length)}</p>
              <div style={{ overflowX: "auto" }}>
