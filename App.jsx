...
function ProjectApp({ project, userRole, user, isSystemOwner, lang, theme, onThemeChange, onLangChange, onBack, onProjectUpdate }) {
  ...
  const exportFlightCSV = () => {
    const fmtTs = (ts) => { const v = toDatetimeLocalValue(ts); return v ? v.replace("T", " ") : ""; };
    const headers = [t.no, t.dept, t.nameKanji, t.nameRoman, t.airline, t.flightNo, t.cabin, t.pnr, t.depAirport, t.depTime, t.arrAirport, t.arrTime, `${t.flightReturn}${t.flightNo}`, `${t.flightReturn}${t.depTime}`, `${t.flightReturn}${t.arrTime}`, t.status];
    const rows = persons.filter((p) => !p.hide_from_flights).map((p) => {
      ...
      return [personIndex[p.id], p.dept, p.name_kanji, `${p.last_roman} ${p.first_roman}`.trim(),
        fl?.airline || "", fl?.flight_no || "", fl?.cabin || "", fl?.pnr || "",
        fl?.dep_airport ? `${fl.dep_airport}${depT ? " "+depT : ""}` : "",
        fmtTs(fl?.dep_time),
        fl?.arr_airport ? `${fl.arr_airport}${arrT ? " "+arrT : ""}` : "",
        fmtTs(fl?.arr_time),
        fl?.ret_flight_no || "",
        fmtTs(fl?.ret_dep_time),
        fmtTs(fl?.ret_arr_time),
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", fontFamily: "'Noto Sans JP',sans-serif" }}>
      ...
      {/* ── Tab bar + Progress ── */}
      <div style={{ background: "var(--shiro)", borderBottom: "1px solid var(--keisenL)" }}>
        ...
        <div style={{ padding: "28px 20px", maxWidth: 1320, margin: "0 auto" }}>
          ...
          {tab === "B" && (
            <div className="tab-content">
              ...
              {showFlightAdv && (
                <div style={{ background: "var(--washi)", padding: "16px 18px", marginBottom: 14, borderRadius: 8, border: "1px solid var(--keisenL)", display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 10 }}>
              ...
              <p style={{ fontSize: 11, color: "var(--nezumi)", marginBottom: 10, letterSpacing: "0.03em" }}>{t.resultsCount.replace("{n}", filteredFlightPersons.length).replace("{total}", persons.filter((p) => !p.hide_from_flights).length)}</p>
              <div style={{ overflowX: "auto" }}>
                <table style={tblW} className="flight-table">
                  <thead><tr style={thead}>
                    {[
                      { key: "no", label: t.no, w: 32 },
                      ...
                      <th key={key} style={{ ...thS, textAlign: "center", minWidth: w, maxWidth: w, width: w }}>
                        {label}
                      </th>
                    ))}
                  </tr></thead>
                    ...
                    {filteredFlightPersons.map((p) => {
                      ...
                      const aptCell = (code, term) => (
                        <div>
                          ...
                          {term && <div style={{ fontSize: 12.5, fontWeight: 400, color: "var(--sumi)" }}>{term}</div>}
                        </div>
                      );
                      return (
                        <tr key={p.id}>
