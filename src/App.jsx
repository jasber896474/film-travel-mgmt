...
const primaryBtn = (disabled) => ({ background:disabled?"#93c5fd":"#2563eb", color:"white", border:"none", borderRadius:8, padding:"8px 18px", fontWeight:700, cursor:disabled?"not-allowed":"pointer", fontSize:13 });

function Toast({ msg }) {
  if (!msg) return null;
  return (
    <div style={{ position:"fixed", top:20, right:20, zIndex:999, background:"#1e3a8a", color:"white", borderRadius:10, padding:"10px 20px", fontSize:13, fontWeight:600, boxShadow:"0 4px 20px rgba(0,0,0,.2)" }}>
      {msg}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
        ...
        <button onClick={() => onSave(f)} style={primaryBtn(false)}>{t.save}</button>
      </div>
    </div>
  );
}
...
function HotelStayForm({ init, hotels, pricingRules, onSave, onClose, t }) {
  ...
  const [f, setF] = useState(init ? { ...blank, ...init, hotel_id:init.hotel_id||"" } : blank);
  const set = k => e => setF(p => ({ ...p, [k]: e.target.value }));
  ...
  const { totalAmount, breakdown } = useMemo(() => {
    ...
    for (let i = 0; i < nights; i++) {
      ...
      const rule = pricingRules.find(r =>
        +r.hotel_id === +f.hotel_id && r.date === ds &&
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-3">
        <Field label={t.staySegment}>
          <input className={inp} value={f.stay_label} onChange={set("stay_label")} placeholder="例：勘景期、拍攝期…" />
        </Field>
        <Field label={t.hotel}>
        ...
        <Field label={t.roomType}>
          <select className={inp} value={f.room_type} onChange={set("room_type")}>{ROOM_TYPES.map(r=><option key={r}>{r}</option>)}</select>
        </Field>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
        <button onClick={onClose} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid #d1d5db", cursor:"pointer" }}>{t.cancel}</button>
        <button onClick={() => onSave({ ...f, nights, total_amount:totalAmount })} style={primaryBtn(false)}>{t.save}</button>
      </div>
    </div>
  );
}
...
function HotelMasterForm({ init, onSave, onClose, t }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
        <button onClick={onClose} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid #d1d5db", cursor:"pointer" }}>{t.cancel}</button>
        <button onClick={() => onSave(f)} style={primaryBtn(!f.name.trim())} disabled={!f.name.trim()}>{t.save}</button>
      </div>
    </div>
  );
}
...
function PricingRuleForm({ init, hotelId, hotelName, onSave, onClose, t }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
        <button onClick={onClose} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid #d1d5db", cursor:"pointer" }}>{t.cancel}</button>
        <button onClick={() => onSave({ ...f, hotel_id:hotelId, final_price:final })} style={primaryBtn(false)}>{t.save}</button>
      </div>
    </div>
  );
}
...
function RoommateModal({ pid, persons, roommates, onSave, onClose, t }) {
  ...
  return (
    <Modal title={`${t.roommateSet} — ${person?.name_kanji||""}`} onClose={onClose}>
      ...
      <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
        <button onClick={onClose} style={{ padding:"8px 16px", borderRadius:8, border:"1px solid #d1d5db", cursor:"pointer" }}>{t.cancel}</button>
        <button onClick={()=>onSave(pid, selected)} style={primaryBtn(false)}>{t.save}</button>
      </div>
    </Modal>
  );
}
...
function MemberManager({ project, user, isOwner, onClose, t }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const rows = await api.get("user_projects", `&project_id=eq.${project.id}`);
      setMembers(rows);
  ...
  const addMember = async () => {
    ...
    setSaving(true);
    try {
      if (members.find(m => m.user_id === userId.trim())) {
        showToast("このユーザーはすでに追加されています"); setSaving(false); return;
      }
  ...
  const removeMember = async (id, uid) => {
    if (uid === user.id) { showToast("自分自身は削除できません"); return; }
    ...
    <Modal title={`👥 ${t.members} — ${project.name}`} onClose={onClose} wide>
      ...
      <div style={{ background:"#f8fafc", borderRadius:10, padding:16, marginBottom:20 }}>
        ...
        <div style={{ fontSize:12, color:"#6b7280", marginBottom:10 }}>
          Supabase → Authentication → Users で確認できる User ID を入力してください。
        </div>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          <input className={inp} value={userId} onChange={e=>setUserId(e.target.value)}
            placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" style={{ flex:2, minWidth:200 }} />
          ...
          <button onClick={addMember} disabled={saving||!userId.trim()} style={primaryBtn(saving||!userId.trim())}>
            {saving?"…":"追加"}
          </button>
        </div>
      </div>
              ...
              <div key={m.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", background:"white", borderRadius:10, border:"1px solid #e5e7eb" }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                    <span style={{ fontSize:11, fontWeight:700, padding:"2px 8px", borderRadius:20, ...(ROLE_COLORS[m.role]||ROLE_COLORS.viewer) }}>
                      {t[`role_${m.role}`] || m.role}
                    </span>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, marginLeft:12, flexShrink:0 }}>
                  {canModify && (
                    <select value={m.role} onChange={e=>changeRole(m.id, e.target.value)}
                      style={{ border:"1px solid #d1d5db", borderRadius:6, padding:"4px 8px", fontSize:12, background:"white", cursor:"pointer" }}>
                      <option value="admin">{t.role_admin}</option>
                      <option value="editor">{t.role_editor}</option>
                      <option value="viewer">{t.role_viewer}</option>
                    </select>
                  ...
                  {canModify && (
                    <button onClick={()=>removeMember(m.id, m.user_id)} style={dBtn}>{t.delete}</button>
                  )}
                </div>
              </div>
            );
      ...
      <div style={{ marginTop:20, padding:"12px 14px", background:"#fefce8", borderRadius:8, border:"1px solid #fde68a" }}>
        <div style={{ fontSize:12, fontWeight:700, color:"#92400e", marginBottom:4 }}>💡 User ID の確認方法</div>
        <div style={{ fontSize:12, color:"#92400e", lineHeight:1.6 }}>
          1. <a href="https://supabase.com/dashboard" target="_blank" rel="noreferrer" style={{ color:"#1d4ed8" }}>Supabase Dashboard</a> を開く →
          Authentication → Users → 対象ユーザーの UUID をコピー
        </div>
      </div>
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1e3a8a,#2563eb)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"white", borderRadius:16, padding:40, width:360, boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <Field label={t.loginEmail}>
          <input className={inp} type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} />
        </Field>
        <Field label={t.loginPw}>
          <input className={inp} type="password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} />
        </Field>
        <button onClick={handleLogin} disabled={loading} style={{ ...primaryBtn(loading), width:"100%", marginTop:8, fontSize:15, padding:12 }}>
          {loading ? "…" : t.loginBtn}
        </button>
  ...
  const load = async () => {
    ...
    try {
      if (isOwner) {
        ...
        setProjects(allProjs.map(p => ({ id:p.id, project_id:p.id, role:"owner", projects:p })));
      } else {
        ...
        setProjects(up.map(r => ({
          ...r,
          projects: projs.find(p=>p.id===r.project_id) || { id:r.project_id, name:"—", description:"" },
        })));
      }
    } catch (e) { showToast("Error: " + e.message); }
    setLoading(false);
    ...
    <div style={{ minHeight:"100vh", background:"#f0f4f8" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1e3a8a,#2563eb)", padding:"16px 24px", color:"white", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div>
          ...
          <div style={{ fontSize:11, opacity:.75 }}>{user.email} · <span style={{ padding:"1px 8px", borderRadius:10, background:"rgba(255,255,255,0.2)", fontSize:10 }}>{isOwner ? t.role_owner : ""}</span></div>
        </div>
      ...
      <div style={{ maxWidth:900, margin:"0 auto", padding:"32px 16px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
          ...
          <button onClick={()=>setShowNew(true)} style={primaryBtn(false)}>{t.newProject}</button>
        </div>
        ...
        {loading ? <div style={{ textAlign:"center", padding:40, color:"#9ca3af" }}>…</div> : (
          projects.length === 0 ? (
            <div style={{ textAlign:"center", padding:60, background:"white", borderRadius:16, boxShadow:"0 2px 8px rgba(0,0,0,.08)" }}>
              ...
              <button onClick={()=>setShowNew(true)} style={primaryBtn(false)}>{t.firstProject}</button>
            </div>
          ) : (
                ...
                <div key={up.id} style={{ background:"white", borderRadius:14, boxShadow:"0 2px 12px rgba(0,0,0,.08)", overflow:"hidden" }}>
                  ...
                  <div style={{ padding:"14px 20px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                      <span style={{ fontSize:11, fontWeight:700, padding:"3px 8px", borderRadius:20, ...(ROLE_COLORS[up.role]||ROLE_COLORS.viewer) }}>
                        {t[`role_${up.role}`] || up.role}
                      </span>
                    ...
                    <div style={{ display:"flex", gap:8 }}>
                      <button onClick={()=>onSelect(up.projects, up.role)}
                        style={{ flex:1, ...primaryBtn(false), padding:"8px", fontSize:13 }}>{t.openProject}</button>
      ...
      {showNew && (
        <Modal title={t.newProject} onClose={()=>setShowNew(false)}>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:16 }}>
            ...
            <button onClick={createProject} disabled={saving||!newName.trim()} style={primaryBtn(saving||!newName.trim())}>
  ...
  const savePerson = async f => {
    try {
      if (personModal.mode === "add") {
        const [r] = await api.insert("persons", { ...f, project_id:pid });
        setPersons(p=>[...p,r]);
      } else {
  ...
  const saveHotel = async f => {
    try {
      if (hotelModal.mode === "add") {
        const [r] = await api.insert("hotels", { ...f, project_id:pid });
        setHotels(h=>[...h,r]);
      } else {
  ...
  const savePricing = async f => {
    try {
      if (priceModal.mode === "add") {
        const [r] = await api.insert("pricing_rules", { ...f, project_id:pid });
    ...
    <div style={{ fontFamily:"system-ui,sans-serif", minHeight:"100vh", background:"#f0f4f8" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1e3a8a,#2563eb)", padding:"14px 24px", color:"white", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <button onClick={onBack} style={{ background:"rgba(255,255,255,0.15)", color:"white", border:"1px solid rgba(255,255,255,.3)", borderRadius:8, padding:"6px 12px", cursor:"pointer", fontSize:13 }}>
            ← {t.back}
          </button>
          <div>
            ...
            <div style={{ fontSize:11, opacity:.75 }}>
              ☁ {user.email} ·
              <span style={{ marginLeft:6, padding:"1px 8px", borderRadius:10, background:"rgba(255,255,255,0.2)", fontSize:10 }}>
                {isOwner ? t.role_owner : (t[`role_${userRole}`] || userRole)}
              </span>
            </div>
          </div>
        </div>
              ...
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, flexWrap:"wrap", gap:8 }}>
                  ...
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                    ...
                    {canEdit && <button onClick={()=>setPersonModal({ mode:"add", data:null })} style={primaryBtn(false)}>{t.add}</button>}
                  </div>
                </div>
                        ...
                        return (
                          <tr key={p.id}>
                            ...
                            <td style={{ ...tdS(i), fontWeight:p.importance===3?700:400, color:p.importance===3?"#dc2626":"inherit" }}>
                              {p.name_kanji}
                              {p.importance===3&&<span style={{ marginLeft:4, fontSize:10, background:"#fee2e2", color:"#dc2626", borderRadius:4, padding:"1px 5px" }}>★★★</span>}
                            </td>
                      ...
                      <div key={p.id} style={{ background:"white", borderRadius:12, boxShadow:"0 2px 8px rgba(0,0,0,.08)", overflow:"hidden" }}>
                        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", background:"#f8fafc", borderBottom:"1px solid #e5e7eb" }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                            ...
                            {canEdit&&(
                              <button onClick={()=>setRoommateModal({ pid:p.id })} style={{ ...eBtn, margin:0 }}>🛏</button>
                            )}
                          </div>
                        </div>
                        {pStays.length===0 ? (
              ...
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:8 }}>
                  ...
                  <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                    ...
                    {canEdit&&<button onClick={()=>setHotelModal({ mode:"add", data:null })} style={primaryBtn(false)}>{t.addHotel}</button>}
                  </div>
                </div>
